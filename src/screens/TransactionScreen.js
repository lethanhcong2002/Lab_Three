...
export default function TransactionScreen({ navigation }) {
  ...
  const handleAccept = async () => {
    if (selectedTransaction) {
      try {
        await TRANS.doc(selectedTransaction.id).update({ status: "Đã xác nhận" });
        setDetailModalVisible(false);
        Alert.alert("Đã xác nhận");
      } catch (error) {
  ...
  const handleDeny = async () => {
    if (selectedTransaction) {
      try {
        await TRANS.doc(selectedTransaction.id).update({ status: "Từ chối" });
        setDetailModalVisible(false);
        ...
        >
          <View style={{ backgroundColor: 'white', padding: 20, margin: 10 }}>
            ...
            <View style={{flexDirection: 'row', justifyContent: 'flex-end', marginVertical: 10}}>
              <Button mode="contained" style={{marginRight: 10 }} onPress={handleAccept}>Xác nhận</Button>
              <Button mode="contained" onPress={handleDeny}>Từ chối</Button>
            </View>
